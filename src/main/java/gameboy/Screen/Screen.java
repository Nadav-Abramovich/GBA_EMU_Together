...
//            System.out.printf("NOT DRAWING: 0x%s, 0x%s\r\n", Integer.toHexString(real_x).toUpperCase(), Integer.toHexString(real_y).toUpperCase());
    ...
    private static void draw_tile(int sprite_pointer, int x, int y, boolean putpixel_by_tile,
                                  boolean is_background, boolean flip_x, boolean flip_y) {
        for (int line = 0; line < 8; line++) {
            for (int col = 0; col < 8; col++) {
                ...
                int window_y = CPU.memory.read_byte(0xFF4A)&255;
                int window_x = (CPU.memory.read_byte(0xFF4B) - 7)&255;
                // Background
                if (is_background) {
                    ...
                    // Window
                    if (putpixel_by_tile) {
                        putPixel(x, y, line + window_y, col + window_x, color);
                    // Object
                    } else {
                        if (flip_y && flip_x) {
    ...
    private static void draw_objects(boolean background_window_over_object) {
        for (char i = 0xFE00; i <= 0xFE9F; i += 4) {
            ...
            if(((attributes & 128) == 0 && !background_window_over_object) || ((attributes & 128) != 0 && background_window_over_object)) {
                if ((attributes & 64) != 0) {
                    flip_y = true;
                }
                if ((attributes & 32) != 0) {
