...
public class Adds implements CPUInstructions {
    ...
    public static void add_hl_bc() {
        add_register(CPU.HL, CPU.BC.getValue(), false, false, false);
    }

    @Opcode(value = 0x19, length = 1, cycles = 2)
    public static void add_hl_de() {
        add_register(CPU.HL, CPU.DE.getValue(), false, false, false);
    }
    ...
    public static void add_sp_s8() {
        add_register(CPU.SP, CPU.memory.read_byte(CPU.PC.getValue() + 1), false, false, false);
    }
}
