...
public class Memory {
    ...
    private final byte[] _hram_memory;
    ...
    public void write(int address, byte value) {
        ...
        if (address == 0xFF46) {
            ...
            hram_bank_number = value;
            return;
        }
        else if(address >= 0xA000 && address <= 0xBFFF) {
            _hram_memory[(0x2000 * hram_bank_number) + (address-0xA000)] = value;
            return;
        }
        if(address >= 0x8000 + (15 * 0x10) && address <= 0x9000)
        {
            int a =1;
        }
...
//            return game_rom[address];
            int base = 0x4000;
            address -= base;
            address = 0x4000 * rom_bank_number + address;
...
//            System.out.printf("Address: 0x%s\r\n", Integer.toHexString(address).toUpperCase());
        ...
        if(address >= 0xE000 && address <= 0xFDFF) {
            System.out.println("ECHO RAM PROHIBITED");
        }
